    ...
    xmlhttp.onreadystatechange = function()
    {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200)
        {
            ...
            for (match of result["matches"])
            {
                ...
                let commentsString = match["comments"].join("!.!")
                ...
                document.getElementById("pastMatchesLeftBox").appendChild(matchData);
            }
        }
    }
...
function addCommentToMatch()
{
    ...
    let url = "/past_matches?matchID=" + matchID + "&comment=" + comment;
    let xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function()
    {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200)
